...
def train_stage2(args, epoch, loader, model, dis_model_global, dis_model_contour, dis_model_head, dis_model_upper, dis_model_lower, multi_netD,\
    ...
    for i, (img, mask, segment, position) in enumerate(loader):
        ...
        img_head = img*segment[:,1:2,:,:] + img*segment[:,2:3,:,:] + img*segment[:,3:4,:,:] + img*segment[:,4:5,:,:]
        img_upper = img*segment[:,5:6,:,:] + img*segment[:,6:7,:,:]
        img_lower = img*segment[:,7:8,:,:] + img*segment[:,8:9,:,:] + img*segment[:,9:10,:,:] + img*segment[:,10:11,:,:] + img*segment[:,11:12,:,:]
        ...
        gen_head_fake = torch.cat([rec_seg[:,1:5,:,:], img_head], dim=1)
        gen_upper_fake = torch.cat([rec_seg[:,5:7,:,:], img_upper], dim=1)
        gen_lower_fake = torch.cat([rec_seg[:,7:12,:,:], img_lower], dim=1)
        ...
        ce_loss = args.alpha_celoss * criterion_seg(rec_seg, torch.argmax(segment, dim=1, keepdim=True).squeeze(1).long()) + \
            args.alpha_coarse_celoss * criterion_seg(coarse_seg, torch.argmax(segment, dim=1, keepdim=True).squeeze(1).long())
        latent_loss = latent_loss.mean()
        ...
        dis_head_real = torch.cat([segment[:,1:5,:,:], img_head], dim=1)
        dis_head_fake = torch.cat([rec_seg[:,1:5,:,:], img_head], dim=1)
        dis_upper_real = torch.cat([segment[:,5:7,:,:], img_upper], dim=1)
        dis_upper_fake = torch.cat([rec_seg[:,5:7,:,:], img_upper], dim=1)
        dis_lower_real = torch.cat([segment[:,7:12,:,:], img_lower], dim=1)
        dis_lower_fake = torch.cat([rec_seg[:,7:12,:,:], img_lower], dim=1)
...
def train_stage3(args, epoch, loader, model, dis_model, optimizer, dis_optimizer, style_loss, perceptual_loss, adversarial_loss, writer):
    ...
    for i, (img, mask, segment, _) in enumerate(loader):
        ...
        for i in range(len(real_feat)):
            fm_loss += criterion_img(fake_feat[i], real_feat[i])
...
def get_arguments():
    ...
    # Prior Train Option
    parser.add_argument('--prior-type', type=str, default='tex')
    ...
    parser.add_argument('--alpha-contour-loss', type=float, default=0)
    parser.add_argument('--alpha-head-loss', type=float, default=0)
    parser.add_argument('--alpha-upper-loss', type=float, default=0)
    parser.add_argument('--alpha-lower-loss', type=float, default=0)
    ...
    parser.add_argument('--alpha-multid-loss', type=int, default=0)
    ...
    # Data Option
    parser.add_argument('--train-image_path', type=str, default='train/')
    parser.add_argument('--train-parsing_path', type=str, default='train/')
    parser.add_argument('--train-id', type=str, default='train_chi.txt')
    parser.add_argument('--val-image-path', type=str, default='val')
    parser.add_argument('--val-parsing-path', type=str, default='val')
    parser.add_argument('--val-id', type=str, default='val_chi.txt')
    # Path Option
    ...
    parser.add_argument('--sample-freq', type=int, default=2500)
    parser.add_argument('--sample-size', type=int, default=8)
    parser.add_argument('--summary-freq', type=int, default=1000)
    ...
    parser.add_argument('--struc-memory-table-id', type=int, default=488)
    parser.add_argument('--tex-memory-table-id', type=int, default=541) # chi_tex_prior_1st_time -> 557
...
if __name__ == '__main__':
    ...
    tensorboard_dir = f'{args.base_tensorboard_path}/{str(args.stage)}/{args.name}/{args.subname}'
    ...
    if args.stage == 1:
        dataset = Dataset_chi(args, args.train_image_path, args.train_parsing_path, args.train_id, augment=True)
        ...
        if args.prior_type == 'struc':
            model = VQVAE(in_channel=(3+12), out_channels=12).cuda()
            ...
            model = VQVAE(in_channel=(3+12), out_channels=3).cuda()
            optimizer = optim.Adam(model.parameters(), lr=args.prior_lr)
    ...
    if args.stage == 2:
        dataset = Dataset_chi(args, args.train_image_path, args.train_parsing_path, args.train_id, augment=True)
        loader = DataLoader(dataset=dataset, batch_size=args.batch_size, num_workers=8, shuffle=True, pin_memory=True, drop_last=True) 
        memory_table_b = torch.from_numpy(np.load(f'{args.memory_path}/quantize_b_{args.struc_memory_table_id}.npy'))
        memory_table_t = torch.from_numpy(np.load(f'{args.memory_path}/quantize_t_{args.struc_memory_table_id}.npy'))
        model = Stage_2_Inner_Constraint(in_channel=(3+12), out_channels=12, norm_layer=args.norm_type, embed_b=memory_table_b, embed_t=memory_table_t, nl=args.nl).cuda()
        dis_model_global = Discriminator(in_channels=(3+12), use_sigmoid=True).cuda()
        dis_model_contour = Discriminator(in_channels=(3+1), use_sigmoid=True).cuda()
        dis_model_head = Discriminator(in_channels=(3+4), use_sigmoid=True).cuda()
        dis_model_upper = Discriminator(in_channels=(3+2), use_sigmoid=True).cuda()
        dis_model_lower = Discriminator(in_channels=(3+5), use_sigmoid=True).cuda()
    ...
    if args.stage == 3:
        dataset = Dataset_chi(args, args.train_image_path, args.train_parsing_path, args.train_id, augment=True)
        loader = DataLoader(dataset=dataset, batch_size=args.batch_size, num_workers=8, shuffle=True, pin_memory=True, drop_last=True)
        memory_table_b = torch.from_numpy(np.load(f'{args.memory_path}/quantize_b_{args.tex_memory_table_id}.npy'))
        memory_table_t = torch.from_numpy(np.load(f'{args.memory_path}/quantize_t_{args.tex_memory_table_id}.npy'))
        model = Stage_3_Inner_Constraint(in_channel=(3+12), norm_layer=args.norm_type, embed_b=memory_table_b, embed_t=memory_table_t, nl=args.nl).cuda()
        dis_model = Discriminator(in_channels=(3+12), use_sigmoid=True).cuda()
